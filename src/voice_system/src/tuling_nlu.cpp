...
int parseJsonResonse(string input)
{
	...
	if(result.compare("GO")==0){
		pubFlag = 2;
	}
	if(result.compare("STOP")==0){
		pubFlag = 3;
	}
...
{
	std::cout<<"my question:"<< msg->data << std::endl;
...
int main(int argc,char* argv[])
{
	...
	while(ros::ok())
	{
		...
		if(pubFlag==2)
		{
			system("play src/voice_system/womenzou.wav");
		...
		if(pubFlag==3)
		{
           	system("play src/voice_system/haode.wav");
			std_msgs::Int32 minglin;
			minglin.data=0;
			pub_1.publish(minglin);
			pubFlag=0;
